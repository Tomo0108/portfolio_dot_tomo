...
const ThreeOrbitingLight: React.FC<ThreeOrbitingLightProps> = ({ width, height, color = 0x4299E1 /* Tailwind Blue-500ish */ }) => {
  ...
  useEffect(() => {
    ...
    textureRef.current = texture; // Store texture in ref
    ...
    materialRef.current = tubeMaterial; // Store material in ref

    const glowingBorder = new THREE.Mesh(tubeGeometry, materialRef.current);
    meshRef.current = glowingBorder; // Store mesh in ref
