...
export function SkillsContainer() {
  ...
  return (
    <div className="relative bg-muted/50 rounded-xl p-6 md:p-8 overflow-hidden">
      ...
      <div className="flex flex-wrap md:flex-nowrap gap-3 mb-8">
        {skills.map((skill, index) => (
          <button
            ...
            className={cn(
              "px-4 py-2.5 rounded-lg flex items-center gap-2.5",
          ...
          {selectedIndex !== null && (
            ...
            >
              <div className="space-y-4">
                <h3 className="text-2xl font-semibold text-primary">{skills[selectedIndex].title}</h3>
                <p className="text-muted-foreground">{skills[selectedIndex].description}</p>
              </div>

              <div className="space-y-4">
                <h4 className="text-lg font-semibold border-b pb-2">実績例</h4>
                <ul className="grid gap-4">
                  {skills[selectedIndex].examples.map((example, index) => (
                    <li 
                      key={index} 
                      className={cn(
                        "bg-background/50 backdrop-blur-sm rounded-lg p-4 space-y-2",
