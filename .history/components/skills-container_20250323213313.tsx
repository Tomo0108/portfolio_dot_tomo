            ...
            <div className="grid gap-4">
              {currentSkill.examples.map((example) => (
                <div
                  key={example.id}
                  className={cn(
                    "bg-background/80 backdrop-blur-sm rounded-lg px-3 pt-3 pb-2 border border-border/50",
                    "transition-all duration-300"
                  )}
                >
                  <Accordion type="single" collapsible>
                    <AccordionItem value={example.id} className="border-none">
                      <AccordionTrigger className="hover:no-underline flex justify-between items-start -ml-1">
                        <div className="grid gap-3">
                          <h5 className="font-japanese font-bold bg-white/90 dark:bg-white/90 text-black px-4 py-2.5 rounded-md text-lg shadow-sm">
                            {example.title}
                          </h5>
                          <p className="text-base text-foreground/90 leading-relaxed px-1">{example.description}</p>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent>
