...
import { ZennArticle } from '@/components/sections/zenn-articles'; // Assuming ZennArticle type is exported
...
const PageFlipArticles: React.FC<PageFlipArticlesProps> = ({ articles }) => {
  ...
  const variants = {
    enter: (direction: number) => ({
      ...
      opacity: 0,
      scale: 0.8,
      transformOrigin: direction > 0 ? 'left center' : 'right center' // Removed trailing comma
    }),
    center: {
      zIndex: 1,
      x: 0,
    ...
    exit: (direction: number) => ({
      ...
      opacity: 0,
      scale: 0.8,
      transformOrigin: direction < 0 ? 'left center' : 'right center', // Ensure this line is correct
      transition: { // Ensure this object is correctly defined
        x: { type: 'spring', stiffness: 300, damping: 30 },
