...
const PageFlipArticles: React.FC<PageFlipArticlesProps> = ({ articles }) => {
  ...
  // useCallbackを早期リターンの前に移動
  const openArticle = useCallback(() => {
    if (currentArticle) { // currentArticleの存在を内部で確認
  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-xl mx-auto aspect-[4/3] sm:aspect-[16/9] md:aspect-[4/2.5] overflow-hidden border-4 border-red-500" style={{ perspective: '1200px' }}>
      <AnimatePresence initial={false} custom={direction} mode="popLayout">
        ...
        >
          <Card className="w-full h-full flex flex-col overflow-hidden shadow-lg group bg-background/80 backdrop-blur-sm rounded-lg border border-border/50 transition-all duration-300 hover:bg-background hover:shadow-xl">
