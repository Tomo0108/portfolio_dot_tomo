...
export function SkillCard({ data, isSelected = false, onSelect, onClose }: SkillCardProps) {
  const [isHovered, setIsHovered] = useState(false);

  return (
    <div
      className={cn(
        "relative p-6 bg-background rounded-lg shadow-sm overflow-hidden group h-full cursor-pointer",
        "transition-all duration-300",
        isSelected ? "md:col-span-3" : ""
      )}
      onClick={() => {
        if (isSelected) {
          onClose?.();
