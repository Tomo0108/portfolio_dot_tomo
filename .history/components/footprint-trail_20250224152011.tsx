...
export function FootprintTrail() {
  ...
  const createFootprintTrail = (startX: number, startY: number, startAngle: number): FootprintPosition[] => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      currentY += stepLength * Math.sin(currentAngle * Math.PI / 180);
      ...
      trail.push({
        ...
        x: currentX,
        y: currentY,
        opacity: 0, // フェードインのため初期値0
        isLeft: isLeft,
        angle: currentAngle + (isLeft ? 0 : 180), // 左右で角度を反転
      });
  ...
  const startNewTrail = async () => {
    const startX = Math.random() * 60 + 20; // 20-80%の範囲
    const startY = Math.random() * 60 + 20;
    const startAngle = Math.random() * 360;
    
    const newTrail = createFootprintTrail(startX, startY, startAngle);
