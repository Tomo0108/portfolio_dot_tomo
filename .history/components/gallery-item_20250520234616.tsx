...
import { ChevronLeft, Circle, Pencil, Palette, Image as ImageIcon, Sparkles } from 'lucide-react';
...
export function GalleryItemView({ item }: { item: GalleryItem }) {
  ...
  const imageSrc = isHighRes
    ? (() => {
        const originalMap = {
          line: item.details.images.originalLine,
          color: item.details.images.originalColor,
          complete: item.details.images.originalComplete
        };
        return originalMap[selectedTab] || imageTypes[selectedTab].src;
      })()
    : imageTypes[selectedTab].src;
              ...
              <div className="space-y-4">
                <div className="relative aspect-[4/3] w-full bg-muted rounded-lg overflow-hidden">
                  ...
                  <Image
                    ...
                    priority
                    onClick={() => setIsHighRes((prev) => !prev)}
                    title={isHighRes ? "標準画質に戻す" : "高画質で表示"}
                    onLoadingComplete={() => setIsLoading(false)}
                    onLoad={() => setIsLoading(true)}
                  />
                  <div className="absolute bottom-2 right-2 bg-white/80 rounded-full p-2 text-primary font-bold pointer-events-none select-none shadow-md flex items-center">
                    {isHighRes ? (
                      <Sparkles className="w-5 h-5 text-primary" title="高画質表示中" />
                    ) : (
                      <ImageIcon className="w-5 h-5 text-primary" title="標準画質" />
                    )}
                  </div>
                </div>
