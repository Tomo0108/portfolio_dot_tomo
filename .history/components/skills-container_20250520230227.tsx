...
export function SkillsContainer() {
  ...
  return (
    <div className="relative bg-background rounded-xl p-0 md:p-0 overflow-hidden">
      <div className="flex flex-wrap md:flex-nowrap gap-4 mb-10 px-6 md:px-8 pt-6 md:pt-8">
        {skills.map((skill, index) => {
          ...
          return (
            <Card
              ...
              className={cn(
                "rounded-lg shadow-lg overflow-hidden cursor-pointer w-full md:w-auto md:flex-1",
                "transition-all duration-300 ease-in-out",
                isActive ? "border-primary bg-card" : "border-border bg-card hover:border-primary/70"
              )}
            >
              <CardContent className="p-4">
                <Badge
                  ...
                  className={cn(
                    "mb-3 px-3 py-1 text-sm font-medium flex items-center gap-2 w-full justify-start",
                    isActive ? "bg-primary text-primary-foreground border-primary" : "bg-secondary text-secondary-foreground border-secondary"
                  )}
                >
                  <Icon className={cn("h-5 w-5", isActive ? "text-primary-foreground" : "text-secondary-foreground")} />
      ...
      <div className="relative min-h-[320px] md:min-h-[280px] px-6 md:px-8 pb-6 md:pb-8">
                        ...
                        <AccordionItem value={example.id} className="border-none">
                          <AccordionTrigger className="hover:no-underline p-4">
                            <div className="w-full flex flex-col items-start gap-3">
                              ...
                              <p className="text-sm text-foreground/90 leading-relaxed text-left">{example.description}</p>
                            </div>
                          </AccordionTrigger>
                          <AccordionContent className="p-4 pt-0">
                            {example.achievements && example.achievements.length > 0 ? (
                              <div className="pt-2 pb-4">
                                <Table className="border border-border">
                                  <TableHeader className="bg-primary">
                                    <TableRow>
                                      <TableHead className="w-[70%] px-4 py-3 text-primary-foreground border-r border-primary-foreground/30">自動化項目</TableHead>
                                      <TableHead className="text-right px-4 py-3 text-primary-foreground">削減時間/month</TableHead>
                                    </TableRow>
                                  </TableHeader>
                                  <TableBody>
                                    {example.achievements.map((achievement, index) => (
                                      <TableRow key={index} className="hover:bg-muted/50 border-b border-border last:border-b-0">
                                        <TableCell className="font-medium p-3 border-r border-border">{achievement.item}</TableCell>
                                        <TableCell className="text-right p-3 text-primary font-semibold">{achievement.reductionTime}</TableCell>
                                      </TableRow>
                                    ))}
                                  </TableBody>
                                </Table>
                              ...
                              <div className="pt-2 px-4 pb-4">
                                {isMobile ? (
                                  <div className="p-8 text-center bg-muted/50 rounded-lg">
                                    <p className="text-sm text-foreground/70 mb-3">PDFをダウンロードして確認する</p>
                                    <a
                                      href={example.pdfUrl}
                                      target="_blank"
                                      rel="noopener noreferrer"
                            ...
                            ) : example.id === 'creative-3' && !example.achievements ? (
                              <div className="pt-2 pb-4 px-4">
                                <p className="text-sm text-foreground/90 leading-relaxed whitespace-pre-line">
                                  {example.longDescription || "詳細な成果情報はありません。"}
                                </p>
                              </div>
                            ) : example.id === 'creative-3' && example.videoUrl ? (
                              <div className="pt-2 px-4 pb-4">
                                <div className="relative overflow-hidden rounded-lg border border-border/50 bg-black/5">
                                  <video
                                    autoPlay
