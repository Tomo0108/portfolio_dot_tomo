...
import { ChevronLeft, Circle, Pencil, Palette, Image as ImageIcon, Sparkles, ZoomIn } from 'lucide-react';
              ...
              <div className="space-y-4">
                <div className="relative aspect-[4/3] w-full bg-muted rounded-lg overflow-hidden">
                  ...
                  <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                    ...
                    <DialogContent>
                      <DialogHeader>
                        <DialogTitle>画質を選択</DialogTitle>
                      </DialogHeader>
                      <div className="flex flex-col gap-4 mt-4">
                        <button
                          className={`px-4 py-2 rounded border font-semibold ${!isHighRes ? 'bg-primary text-white' : 'bg-background text-primary border-primary'}`}
                          onClick={() => { setIsHighRes(false); setDialogOpen(false); }}
                        >
                          標準画質で表示
                        </button>
                        <button
                          className={`px-4 py-2 rounded border font-semibold ${isHighRes ? 'bg-primary text-white' : 'bg-background text-primary border-primary'}`}
                          onClick={() => { setIsHighRes(true); setDialogOpen(false); }}
                        >
                          高画質で表示
                        </button>
                      </div>
