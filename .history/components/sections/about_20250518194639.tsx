...
export function About() {
  const [selected, setSelected] = useState<'technical' | 'creative' | 'strategy'>('technical');
          ...
          >
            ...
            <div className="mt-8">
              <div className="flex gap-2 mb-4">
                <button
                  className={`flex items-center gap-2 px-4 py-2 rounded-t-xl font-medium border transition-colors shadow-sm ${selected === 'technical' ? 'bg-primary text-white border-primary/80' : 'bg-muted text-foreground border-border/50 hover:bg-primary/20'}`}
                  onClick={() => setSelected('technical')}
                >
                  <Code2 className="h-5 w-5" /> Technical Direction
                </button>
                <button
                  className={`flex items-center gap-2 px-4 py-2 rounded-t-xl font-medium border transition-colors shadow-sm ${selected === 'creative' ? 'bg-primary text-white border-primary/80' : 'bg-muted text-foreground border-border/50 hover:bg-primary/20'}`}
                  onClick={() => setSelected('creative')}
                >
                  <Palette className="h-5 w-5" /> Creative Direction
                </button>
                <button
                  className={`flex items-center gap-2 px-4 py-2 rounded-t-xl font-medium border transition-colors shadow-sm ${selected === 'strategy' ? 'bg-primary text-white border-primary/80' : 'bg-muted text-foreground border-border/50 hover:bg-primary/20'}`}
                  onClick={() => setSelected('strategy')}
                >
                  <Lightbulb className="h-5 w-5" /> Strategy
                </button>
              </div>
              <div className="bg-background/90 rounded-b-xl px-4 py-3 text-base font-semibold text-foreground shadow-sm border-x border-b border-primary/40 -mt-1 min-h-[56px] flex items-center">
                {selected === 'technical' && (
                  <span>AIを活用したWebアプリケーションの開発で、業務効率を飛躍的に向上させます</span>
                )}
                {selected === 'creative' && (
                  <span>デザイン性とユーザー体験を両立したWebアプリケーションを提案・実現します</span>
                )}
                {selected === 'strategy' && (
                  <span>戦略的な視点で課題を分析し、最適なソリューションを導きます</span>
                )}
              </div>
            </div>
          </motion.div>
