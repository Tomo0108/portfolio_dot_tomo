...
export interface Example {
  ...
  title: string;
  description: string;
  longDescription?: string; // オプショナルに変更
  achievements?: Achievement[]; // 新しいプロパティ
  videoUrl?: string;
  pdfUrl?: string;
}
...
export const skills: Skill[] = [
  {
    ...
    examples: [
      ...
      },
      {
        ...
        achievements: [
          { item: "管理シートからのデータ転記処理を自動化", reductionTime: "5h" },
          { item: "請求データ作成時の日付書き換え処理の自動化", reductionTime: "1h" },
          { item: "請求データ作成FMTの最新FMTでのデータ転記作業を自動化", reductionTime: "1.5h" },
          { item: "工事完了確認作業の効率化ツールの作成", reductionTime: "1h" }
        ]
      }
    ]
  },
